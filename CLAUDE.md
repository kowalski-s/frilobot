...
Этап 1 — Фундамент. Фаза 5 завершена. Переход к Фазе 6.
...
- Фаза 5.1: bot/keyboards/settings.py — клавиатуры настроек (лимит, тихие часы, задержки + возврат), bot/states/settings.py — SettingsEditState (2 состояния), bot/handlers/settings.py — просмотр настроек, изменение лимита (inline-кнопки 3/5/10/15), тихих часов (текст «23:00-08:00»), задержек (текст «30-120»)

### Что дальше
- Фаза 6: модуль «Радар» (парсер tgstat, поиск каналов, подключение)
