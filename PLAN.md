...
- `bot/states/onboarding.py` — `OnboardingState(StatesGroup)`:
  - `welcome`, `specialization`, `specialization_custom`, `services`, `search_keywords`, `search_budget`, `search_format`, `disclaimer`
- `bot/keyboards/onboarding.py`:
  - `get_specialization_keyboard()` — мультивыбор: Разработка, Дизайн, Маркетинг/SMM, Копирайтинг, Автоматизация/боты + «Другое (ввести своё)» + «Далее»
...
- `bot/handlers/start.py` — расширить:
  ...
  - Шаг 2 (specialization): мультивыбор специализаций + «Другое» для ввода своей, сохранение в state data
...
- `bot/keyboards/profile.py`:
  - `ProfileCallback(CallbackData)` — prefix `prof`, fields `action`, `value`
  - `get_profile_keyboard()` — кнопки: Специализации, Описание услуг, Параметры поиска, ← Меню
  - `get_profile_back_keyboard()` — кнопка возврата в профиль
...
**Файлы:**
- `bot/states/profile.py` — `ProfileEditState(StatesGroup)`: 6 состояний для редактирования полей
- `bot/handlers/profile.py`:
  - Показ профиля: специализации, описание, ключевые слова, бюджет, формат
  - Редактирование специализаций: мультивыбор (переиспользование клавиатур онбординга) + пользовательские
  - Редактирование описания услуг: ввод текстом
  - Редактирование параметров поиска: ключевые слова → бюджет → формат (3-шаговый FSM)
  - Все изменения сохраняются в БД (users + search_profiles)
- `bot/handlers/__init__.py` — подключён `profile_router` (перед `menu_router` для приоритета)
...
- Нажать «Описание услуг» → ввести новый текст → сообщение «Обновлено»
